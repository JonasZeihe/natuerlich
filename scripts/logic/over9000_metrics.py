...
COMMENT_LANGS = {".java", ".kt", ".kts", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"}

DEFAULT_SCAN_ROOTS = ["backend/src", "frontend/src"]
DEFAULT_EXCLUDE_PREFIXES = ["scripts/", "docs/", "legacy_archive/"]
...
def parse_git_log_numstat(repo_root: str, roots: List[str]) -> List[GitCommitStat]:
    ...
    for l in lines:
        ...
        a, d, p = parts[0], parts[1], path_norm(parts[2])
        ...
        if p.startswith("backend/src/"):
            ins_backend += ai
            del_backend += di
        elif p.startswith("frontend/src/"):
            ins_frontend += ai
            del_frontend += di
...
def git_ls_tree_code_summary(
    ...
    backend = {"files": 0, "bytes": 0}
    frontend = {"files": 0, "bytes": 0}
    ...
    return {"total": total, "backend": backend, "frontend": frontend}
...
def monthly_rollup(commits: List[GitCommitStat]) -> List[MonthStat]:
    ...
    for mk, cs in items:
        ...
        net = ins - dele
        churn = ins + dele
        be_churn = sum((c.ins_backend + c.del_backend) for c in cs)
        fe_churn = sum((c.ins_frontend + c.del_frontend) for c in cs)
        dominant = (
            "BACKEND"
            if be_churn > fe_churn
            else ("FRONTEND" if fe_churn > be_churn else "BALANCED")
        )
        out.append(
            {
                "period": mk,
...
def build_growth_snapshots(
    ...
    cum_be = 0
    cum_fe = 0
    cum_map: Dict[str, Tuple[int, int, int]] = {}

    for c in commits_chrono:
        cum_total += c.ins_total - c.del_total
        cum_be += c.ins_backend - c.del_backend
        cum_fe += c.ins_frontend - c.del_frontend
        cum_map[c.sha] = (cum_total, cum_be, cum_fe)
    ...
    for key, end_dt in period_keys:
        ...
        loc_total, loc_be, loc_fe = cum_map.get(last_sha, (0, 0, 0))
        ...
        out.append(
            {
                ...
                "files_be": int(tree["backend"]["files"]),
                "bytes_be": int(tree["backend"]["bytes"]),
                "files_fe": int(tree["frontend"]["files"]),
                "bytes_fe": int(tree["frontend"]["bytes"]),
                "loc_approx_total": int(loc_total),
                "loc_approx_be": int(loc_be),
                "loc_approx_fe": int(loc_fe),
            }
        )
...
def folder_bucket(path: str) -> str:
    p = path_norm(path)
    if p.startswith("backend/src/"):
        rest = p[len("backend/src/") :]
        return "backend/src/" + rest.split("/", 1)[0] if "/" in rest else "backend/src"
    if p.startswith("frontend/src/"):
        rest = p[len("frontend/src/") :]
        ...
        if top in (
            ...
            if top == "providers.tsx":
                return "frontend/src/providers.tsx"
            return f"frontend/src/{top}"
...
def test_maturity(backend_files: List[FileStats]) -> Dict[str, Any]:
    main_files = [
        f for f in backend_files if path_norm(f.path).startswith("backend/src/main/")
    ]
    test_files = [
        f for f in backend_files if path_norm(f.path).startswith("backend/src/test/")
    ]
    main_agg = aggregate_file_stats(main_files)
    test_agg = aggregate_file_stats(test_files)

    main_files_n = int(main_agg["files"])
    test_files_n = int(test_agg["files"])
    main_loc = int(main_agg["loc_code"])
    test_loc = int(test_agg["loc_code"])

    return {
        "main_files": main_files_n,
        "test_files": test_files_n,
        "main_loc": main_loc,
        "test_loc": test_loc,
        "file_ratio": ratio_str(test_files_n, max(1, main_files_n)),
        "loc_ratio": ratio_str(test_loc, max(1, main_loc)),
    }


def build_report(repo_root: str) -> str:
    now = iso_utc_now()
    ...
    LOG.info("Computing current working tree stats for src...")
    ...
    backend_files = [
        f for f in file_stats if path_norm(f.path).startswith("backend/src/")
    ]
    frontend_files = [
        f for f in file_stats if path_norm(f.path).startswith("frontend/src/")
    ]

    total_agg = aggregate_file_stats(file_stats)
    backend_agg = aggregate_file_stats(backend_files)
    frontend_agg = aggregate_file_stats(frontend_files)

    LOG.info("Parsing git history (path-limited to src)...")
    ...
    be_share = ratio_str(
        int(backend_agg["loc_code"]), max(1, int(total_agg["loc_code"]))
    )
    fe_share = ratio_str(
        int(frontend_agg["loc_code"]), max(1, int(total_agg["loc_code"]))
    )

    maturity = test_maturity(backend_files)
    ext_compact = ext_breakdown_compact(file_stats, top_n=6)
    scope_compact = scope_map(file_stats)[:10]

    lines: List[str] = []
    lines.append("# Shizen Over9000 Metrics Report\n\n")
    ...
    lines.append(f"- Started (first src-touch): `{started}`\n")
    lines.append(f"- Latest (last src-touch): `{latest}`\n")

    scale = (
        f"- Src scale now: `{total_agg['files']}` files, `{total_agg['loc_code']}` "
        f"code lines, `{human_bytes(int(total_agg['bytes']))}`\n"
    )
    lines.append(scale)

    split = (
        f"- Fullstack split (code lines): Backend `{backend_agg['loc_code']}` ({be_share}) "
        f"vs Frontend `{frontend_agg['loc_code']}` ({fe_share})\n"
    )
    lines.append(split)

    activity = (
        f"- Git activity (src): `{len(commits)}` commits, `{continuity['active_days']}` "
    ...
    if peak_by_churn is not None:
        lines.append(
            ...
            f"dominant `{peak_by_churn['dominant']}`)\n"
        )
    lines.append("\n")

    lines.append("## Scope\n")
    lines.append("- Included: `backend/src/**`, `frontend/src/**`\n")
    lines.append("- Excluded (hard): `scripts/**`, `docs/**`, `legacy_archive/**`\n")
    lines.append(
        "- File discovery: `git ls-files` (tracked + untracked, `--exclude-standard`, "
        "nested `.gitignore`)\n\n"
    )

    lines.append("## Momentum (src)\n")
    ...
    lines.append("## Executive Summary (src now)\n")
    summary_rows = [
        [
            "TOTAL",
            f"{total_agg['files']}",
        ...
        [
            "BACKEND",
            f"{backend_agg['files']}",
            human_bytes(int(backend_agg["bytes"])),
            f"{backend_agg['loc_total']}",
            f"{backend_agg['loc_code']}",
            f"{backend_agg['loc_comment']}",
            f"{backend_agg['loc_blank']}",
        ],
        [
            "FRONTEND",
            f"{frontend_agg['files']}",
            human_bytes(int(frontend_agg["bytes"])),
            f"{frontend_agg['loc_total']}",
            f"{frontend_agg['loc_code']}",
            f"{frontend_agg['loc_comment']}",
            f"{frontend_agg['loc_blank']}",
        ],
    ]
    lines.append(
        md_table(
    ...
    lines.append("## Engineering Maturity (signals)\n")
    lines.append(
        f"- Backend tests: `{maturity['test_files']}` test files vs `{maturity['main_files']}` "
        f"main files (ratio `{maturity['file_ratio']}`)\n"
    )
    lines.append(
        f"- Backend tests LOC: `{maturity['test_loc']}` test code lines vs `{maturity['main_loc']}` "
        f"main code lines (ratio `{maturity['loc_ratio']}`)\n"
    )
    comment_ratio = ratio_str(
        int(total_agg["loc_comment"]), max(1, int(total_agg["loc_total"]))
    )
    lines.append(
        f"- Comment density (rough): `{total_agg['loc_comment']}` comment lines out of "
        f"`{total_agg['loc_total']}` total lines ({comment_ratio})\n"
    )
    lines.append("\n")
        ...
        for g in growth:
            rows.append(
                [
                    ...
                    f"BE {g['loc_approx_be']}",
                    f"FE {g['loc_approx_fe']}",
                ]
            )
        lines.append(
            md_table(
    ...
    lines.append("## Appendix: Biggest Commits (by churn, src only)\n")
    big_commits = sorted(
    ...
    for c in big_commits:
        ...
        rows.append(
            [
                ...
                f"BE +{c.ins_backend}/-{c.del_backend}",
                f"FE +{c.ins_frontend}/-{c.del_frontend}",
                subject,
            ]
        )
    lines.append(
    ...
    lines.append(
        "- These are not “bad” by default; they are prime targets for modularization "
        "if you want even more maintainability.\n\n"
    )
    rows = []
    all_big = sorted(
